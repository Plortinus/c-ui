...
export function HSVaColor(h = 0, s = 0, v = 0, a = 1) {
	...
	const that = {
		...
		toHSVA() {
			...
			hsva.toString = mapper(
				hsva,
				(arr) => `hsva(${arr[0]}, ${arr[1]}%, ${arr[2]}%, ${that.a})`
			)
			return hsva
		},
		...
		toHSLA() {
			...
			hsla.toString = mapper(
				hsla,
				(arr) => `hsla(${arr[0]}, ${arr[1]}%, ${arr[2]}%, ${that.a})`
			)
			return hsla
		},
		...
		toRGBA() {
			...
			rgba.toString = mapper(
				rgba,
				(arr) => `rgba(${arr[0]}, ${arr[1]}, ${arr[2]}, ${that.a})`
			)
			return rgba
		},
		...
		toCMYK() {
			...
			cmyk.toString = mapper(
				cmyk,
				(arr) => `cmyk(${arr[0]}%, ${arr[1]}%, ${arr[2]}%, ${arr[3]}%)`
			)
			return cmyk
		},
