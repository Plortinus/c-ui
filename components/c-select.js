...
export default class CSelect extends HTMLElement {
	...
	constructor() {
        ...
        <c-popover id="root" ${this.search ? 'accomplish' : ''}>
            <c-tips id="tip" type="error">
                <${
									this.search ? 'c-input' : 'c-button'
								} id="select" debounce="200" readonly ${
				...
				if (value === '') {
					...
					this.nodes = [
						...this.querySelectorAll(
							`c-option[key*="${value}" i]:not([disabled])`
						),
					]
					this.filter.textContent = `
	...
	set value(value) {
		...
		if (value !== this.value) {
			...
			const cur =
				this.querySelector(`c-option[value="${value}"]`) ||
				this.querySelector(`c-option`)
