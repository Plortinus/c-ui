...
export default class CPagination extends HTMLElement {
  ...
  render(pagesize, total) {
    ...
    if (this.simple) {
      ...
      const html = Array.from({ length: this.count }, (el, i) => i)
        .splice(0, 9)
        .map(
          (el) =>
            `<c-button ${this.href ? "href=" + (el + 1) : ""} target="_self" ${
              el + 1 == current ? "current" : ""
            } type="flat" data-current="${el + 1}">${el + 1}</c-button>`
        )
        .join("");
      this.page.innerHTML = html;
    }
  ...
  updatePage(current = this.current) {
    ...
    if (this.simple) {
      this.page.querySelector(".simple-page").textContent =
        current + " / " + this.count;
    } else {
      if (this.count > 9) {
        let place = [];
