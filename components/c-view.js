  ...
  connectedCallback() {
    ...
    if (this.draggable) {
      ...
      document.addEventListener("dragover", (ev) => {
        if (this.cloneObj) {
          ...
          this.cloneObj.style.transform = `translate3d( ${left}px ,${parseInt(
            top
          )}px,0)`;
        }
      });
    ...
    if (this.resizable) {
      ...
      resizeCon.addEventListener("mousedown", (ev) => {
        ...
        transforms =
          transform === "none" ? [0, 0] : transform.match(/[\d]+/g).slice(4);
        if (path[0].tagName === "I") {
          this.resizing = true;
          startX = ev.pageX;
          startY = ev.pageY;
      ...
      document.addEventListener("mousemove", (ev) => {
        if (this.resizing) {
          ...
          this.style.transform = `translate3d(${
            Number(transforms[0]) + offsetX
          }px,${Number(transforms[1]) + offsetY}px,0)`;
          this.dispatchEvent(
