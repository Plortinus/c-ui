...
export default class CButton extends HTMLElement {
    ...
    constructor() {
        ...
        */
        ...
        <${this.href?'a':'button'} ${this.htmltype?'type="'+this.htmltype+'"':''} ${(this.download&&this.href)?'download="'+this.download+'"':''} ${this.href?'href="'+this.href+'" target="'+this.target+'" rel="'+this.rel+'"':''} class="btn" id="btn"></${this.href?'a':'button'}>${!this.loading && this.icon && this.icon!='null'?'<c-icon id="icon" name='+this.icon+'></c-icon>':''}<slot></slot>
        `
    }
    ...
    connectedCallback() {
        ...
        this.load = document.createElement('c-loading');
...
class CButtonGroup extends HTMLElement {
    ...
    constructor() {
        ...
        ::slotted(c-button:not(:first-of-type):not(:last-of-type)){
            border-radius:0;
        }
        ::slotted(c-button){
            margin:0!important;
        }
        ::slotted(c-button:not(:first-of-type)){
            margin-left:-1px!important;
        }
        ::slotted(c-button[type]:not([type="dashed"]):not(:first-of-type)){
            margin-left:1px!important;
        }
        ::slotted(c-button:first-of-type){
            border-top-right-radius: 0;
            border-bottom-right-radius: 0px;
        }
        ::slotted(c-button:last-of-type){
...
if(!customElements.get('c-button-group')){
    customElements.define('c-button-group', CButtonGroup);
}
